...
states_sf <- st_read("states_tiger_shapefiles_ftp/cb_2017_us_state_500k.shp")
states_sf
glimpse(states_sf)
...
# read in shapefile as sf
counties_sf <- st_read("counties_tiger_shapefiles_ftp/cb_2017_us_county_500k.shp")
counties_sf
glimpse(counties_sf)
...
# create rotation function for use in transform_points function
rotation = function(a){
        r = a * pi/180 #degrees to radians
        matrix(c(cos(r), sin(r), -sin(r), cos(r)), nrow = 2, ncol = 2)
} 

# create transform_points function
# for shift, format is c(horizontal (more negative is west), vertical (more negative is south))
transform_points <- function(points_sf, shift = c(0, 0), scale = 1, rotate_degrees = 0) {
        ...
        # transform by shift
        points_transformed_sfc <- (points_transformed_sfc - points_centroid_sfc) + shift + points_centroid_sfc
        
        # get centroid for points after shift
        points_centroid_sfc <- st_centroid(points_transformed_sfc)
        
        # transform by scale
        points_transformed_sfc <- (points_transformed_sfc - points_centroid_sfc) * scale + points_centroid_sfc
        
        # get centroid for points after scale
        points_centroid_sfc <- st_centroid(points_transformed_sfc)
        
        # transform by rotation
        points_transformed_sfc <- (points_transformed_sfc - points_centroid_sfc) * rotation(rotate_degrees) +
                points_centroid_sfc
...
# test transform_points
# points_sf <- states_sf %>% filter(NAME == "California")
# points_sf <- states_sf %>% filter(NAME == "California")
# scale <- .3
# shift <- c(100, 100)
# rotate <- 90
# points_sf %>% transform_points(points_sf = ., scale = scale, shift = shift, rotate_degrees = rotate) %>%
#         ggplot() + geom_sf()
...
alaska_state <- states_sf %>% filter(NAME == "Alaska") %>%
        ...
        transform_points(points_sf = ., shift = c(820000, -4730000), scale = .35, rotate_degrees = -50) %>%
        identity()
alaska_state
...
alaska_counties <- counties_sf %>% filter(STATEFP == "02") %>%
        # st_crop(c(xmin = -4000000, ymin = 1466024, xmax = -1512211, ymax = 3909687)) %>%
        # transform_points(points_sf = ., shift = c(820000, -4730000), scale = .35, rotate_degrees = -50) %>%
        transform_points(points_sf = ., shift = c(0, 0), scale = 1, rotate_degrees = -50) %>%
        identity()
alaska_counties
...
hawaii_state <- states_sf %>% filter(NAME == "Hawaii") %>%
        ...
        transform_points(points_sf = ., shift = c(5000000, -1150000), scale = 1, rotate_degrees = -35) %>%
        identity()
hawaii_state
...
# combine transformed geographies
states_sf_tranformed <- states_sf %>% filter(!(NAME %in% c("Alaska", "Hawaii"))) %>%
        rbind(alaska_state) %>%
        rbind(hawaii_state) %>%
        identity()

# remove unused geographies
states_sf_tranformed <- states_sf_tranformed %>% filter(!(NAME %in% c("American Samoa", "Guam", 
...
states_sf_tranformed <- states_sf_tranformed %>% mutate(NAME = as.character(NAME))
states_sf_tranformed <- states_sf_tranformed %>% left_join(., arrests, by = c("NAME" = "state"))
glimpse(states_sf_tranformed)
...
states_map <- states_sf_tranformed %>% 
        st_transform(102003) %>%
        ggplot() + geom_sf(aes(fill = Murder)) + coord_sf(crs = st_crs(102003)) +
        ...
        theme(
              ...
              rect = element_blank(),
              text = element_blank()
            ) + ggtitle("U.S. murder rate by state")
